...
// Output is machine-consumed; prettier costs ~half the total runtime, so it's
// opt-in for humans reading .generated/.
const PRETTY = process.env.GENERATE_PRETTY === "1";
...
const writeStats = { written: 0, skipped: 0 };
const outputPaths = new Set();

// Skipping unchanged files keeps mtimes stable so watchers and incremental
// tsc see fewer dirty inputs.
...
function pruneStaleOutputs(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const entryPath = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      pruneStaleOutputs(entryPath);
      if (fs.readdirSync(entryPath).length === 0) fs.rmdirSync(entryPath);
    } else if (!outputPaths.has(path.resolve(entryPath))) {
      fs.rmSync(entryPath);
    }
  }
}
...
loadSchemas().then(async (schemas) => {
  ...
  pruneStaleOutputs(GENERATED_DIR);
  ...
  console.log(
    `  - ${writeStats.written} files written, ${writeStats.skipped} unchanged`,
  );
});
