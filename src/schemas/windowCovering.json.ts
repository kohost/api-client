import { type Definitions } from "./definitions.json";

export const wcDisciminiators = ["basic", "variable"] as const;

export const schema = {
  $schema: "http://json-schema.org/draft-07/schema",
  $id: "windowCovering.json",
  title: "Window Covering",
  description: "Any smart window covering",
  type: "object",
  properties: {
    id: {
      $ref: "definitions.json#/definitions/id",
    },
    name: {
      type: "string",
    },
    type: {
      $ref: "definitions.json#/definitions/type",
      default: "windowCovering",
    },
    discriminator: {
      type: "string",
      enum: wcDisciminiators,
      default: "variable",
    },
    supportedNotifications: {
      $ref: "definitions.json#/definitions/supportedNotifications",
    },
    notification: {
      $ref: "definitions.json#/definitions/notification",
    },
    driver: {
      $ref: "definitions.json#/definitions/driver",
    },
    offline: {
      type: "boolean",
    },
    position: {
      type: ["number", "null"],
      minimum: 0,
      maximum: 100,
    },
    systemId: {
      $ref: "definitions.json#/definitions/systemId",
    },
    watts: {
      $ref: "definitions.json#/definitions/watts",
    },
  },
  required: ["id", "type", "position", "driver"],
} as const;

type WindowCoveringDiscriminator = (typeof wcDisciminiators)[number];

export interface WindowCoveringSchema {
  id: Definitions["id"];
  name?: string;
  type: "windowCovering";
  discriminator: WindowCoveringDiscriminator;
  supportedNotifications?: Definitions["supportedNotifications"];
  notification?: Definitions["notification"];
  driver: Definitions["driver"];
  offline?: boolean;
  position: number | null;
  systemId?: Definitions["systemId"];
  watts?: Definitions["watts"];
}
