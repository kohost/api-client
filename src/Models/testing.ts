...
interface ValidateableEntity {
  validator(data: any): boolean | void;
}
...
export abstract class Entity<Schema> implements ValidateableEntity {
  data: Schema;
  ...
  abstract validator(data: Schema): boolean | void;
  static validProperties: string[];
  get schemaProperrties() {
    return Object.keys(this).filter((key) =>
      Entity.validProperties.includes(key)
    );
  }
}
...
class ThermostatModel extends Entity<Thermostat> {
  ...
  static validProperties = Object.keys(thermostatSchema.properties);
}
